...
const mode =
  process.argv.length > 2
    ? process.argv[process.argv.length - 1].toLowerCase()
    : MODE.DEFAULT;
...
const emulator =
  flowConfig.emulators[mode === MODE.TEST ? MODE.TEST : MODE.DEFAULT];
...
(async () => {
  ...
  if (mode === MODE.DEFAULT || mode === MODE.TEST) {
    ...
    waitOn(opts).then(async () => {
      ...
      updateConfiguration();
      processContractFolders(["Flow"]).then(() => {
        if (mode === MODE.DEFAULT) {
          spawn(
            "npx",
            [
              "watch",
              `node ${path.join(__dirname, SCRIPT_NAME)} deploy`,
              "contracts/Project",
            ],
            { stdio: "inherit" }
          );
        } else if (mode === MODE.TEST) {
          processContractFolders(["Project"]).then(async () => {
            await transpile();
            spawn.sync(
              "npx",
              [
                ...
                "--timeout",
                "20000",
                path.join(__dirname, "..", "..", "dapplib", "tests"),
              ],
              {
                stdio: "inherit",
              }
    ...
    processContractFolders(["Project"]).then(() => {
      spawn(
        "npx",
        [
          "watch",
          `node ${path.join(__dirname, SCRIPT_NAME)} transpile`,
          "interactions",
        ],
        { stdio: "inherit" }
      );
    ...
    return new Promise((resolve, reject) => {
      ...
      emitter.on("file", (filePath) => {
        for (let f = 0; f < folders.length; f++) {
          if (
            filePath.endsWith(".cdc") &&
            filePath.indexOf(`/contracts/${folders[f]}/`) > -1
          ) {
            let { code, contractNames, deps } = getContractDependencies(
              folders[f],
              filePath
            );
  ...
  function getContractDependencies(contractType, filePath) {
    ...
    const contractRegex =
      /\scontract\s+(interface\s+)?(?<contract>[a-zA-Z0-9_]+)\s*\:?.*\s*{\s/gm;
  ...
  function deployContracts(callback) {
    ...
    let handle = setInterval(async () => {
      ...
      if (item !== null) {
        item.contract = Flow.replaceImportRefs(
          item.contract,
        ...
        console.log(`    ✅  ${item.contractName} => ${contractAddress}`);
        dappConfig.contracts[
  ...
  async function transpile(runTest) {
    if (fs.existsSync(dappConfigFile)) {
      ...
      let interactionsFolder = path.join(
        __dirname,
        "..",
        "..",
        "dapplib",
  ...
  async function generate(
    ...
    return new Promise((resolve, reject) => {
      ...
      outSource +=
        ...
        (isTransaction ? "Transactions" : "Scripts") +
        " {" +
        NEWLINE;
      ...
      emitter.on("file", (filePath) => {
        if (filePath.endsWith(".cdc")) {
          ...
          // Function name
          outSource +=
            NEWLINE + TAB + "static " + functionName + "() {" + NEWLINE;
          ...
          outSource +=
            ...
            TAB +
            "return fcl." +
            (isTransaction ? "transaction" : "script") +
            "`" +
            NEWLINE;
      ...
      emitter.on("end", () => {
        ...
        fs.writeFileSync(
          path.join(destFolder, "dapp-" + type + ".js"),
          outSource,
          "utf8"
        );
